...
<template>
  <el-dialog v-model="visible" width="30%" align-center append-to-body class="logout">
    <template #header>
      <span class="title"> {{ $t('user.safeLogout') }} </span>
    </template>
    <div class="content">
      <p>{{ $t('user.sureToLogout') }}</p>
    </div>
    <template #footer>
      <div class="dialog-footer">
        <el-button class="btn-left" @click="btnComfirmEvent">{{ $tc('home.confirm', 2) }}</el-button>
