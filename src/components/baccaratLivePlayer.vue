<template>
  <div class="baccarat-live-player"  :class="{ 'big-size': playerBoxShowBig }" @click="playerScale('')">
    <div id="player-contianer"></div>
  </div>
</template>
<script setup>
import tool from '@/utils/tool';
import { onMounted, ref, toRefs, inject, onUnmounted, watch, nextTick } from 'vue'
...
function playerInit() {
  ...
  watch(() => player.value && player.value.getStatus(), val => {
    ...
    if (val === 'paused' && playerError.value === "fetchError") {
      ...
      setTimeout(() => {
        try {
          if (player.value && player.value.getStatus() === 'paused') {
            toPlay()
          }
        } catch (error) {
          console.log('watch-播放器状态error', error);
        }
      }, 1500);
    }
  })
...
onMounted(() => {
  liveUrl.value = tool.local.get("roomDetail").pull_address || ""
  if (liveUrl.value) {
    playerInit()
    setTimeout(() => {
      toPlay()
...
// 添加缩放标识
const playerBoxShowBig = ref(false)

// 缩放
function playerScale(size) {
  playerBoxShowBig.value = (() => {
    if (size && size === "big") {
      return true
    }
    if (size && size === "small") {
...
defineExpose({ destoryPlayer, resize, playerScale })
...
.baccarat-live-player {
  ...
  &.big-size {
    transform: scale(1.56) translateZ(0px);
  }
